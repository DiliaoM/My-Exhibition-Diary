# 🖼️ My Exhibition Diary

**My Exhibition Diary** is a personal project for recording, organizing, and revisiting art exhibitions from around the world.  
It's designed as a minimalist, private space for art lovers to collect and track exhibitions they've attended or wish to see — current, upcoming, or past.

> ✨ Think of it as a personalized "exhibition wishlist" and memory archive.

---


## 🌐 Live Site

[https://my-exhibition-diary.onrender.com/](https://my-exhibition-diary.onrender.com/)

---

## 🔍 Features

- 📌 Save exhibitions with details like title, museum, time period, and link
- 📂 Organize into sections like *Upcoming*, *Visited*, or *Wishlist*
- 🖼️ Clean, gallery-style layout
- 🌐 Exhibition links open to official museum pages
- 🛡️ Private-first — user registration and public browsing are disabled by default

---

## 🛠️ Stack

- **Frontend:** TypeScript + React
- **Backend:** Node.js (REST API) + Prisma (PostgreSQL)

---

## 📷 Screenshots



- Homepage with exhibition cards
<img width="1672" height="966" alt="Screenshot 2025-07-19 at 16 20 04" src="https://github.com/user-attachments/assets/f82992fe-ac7c-4d45-88ee-98cd50517049" />

- Add/Edit exhibition modal

<img width="954" height="945" alt="Screenshot 2025-07-19 at 16 21 24" src="https://github.com/user-attachments/assets/f0e66cbb-4c5b-4100-ae1a-87f445d48723" />

- Admin dashboard

  <img width="1233" height="950" alt="Screenshot 2025-07-19 at 16 22 31" src="https://github.com/user-attachments/assets/c9c22250-e57a-4ac9-b0e8-38a0f0a20be0" />

- Feedback

<img width="1249" height="863" alt="Screenshot 2025-07-19 at 16 23 54" src="https://github.com/user-attachments/assets/b8a9d3d8-ea36-4421-9bc5-0e39114b40a0" />


---

## 🧠 Future Plans

- 🌍 Enable public access and allow everyone to create and share their exhibition journals
- 📅 Timeline / calendar view of visits
- 🧩 Add a "Daily Art Quiz" or “Art Fact of the Day” section — fun, interactive content
- 🤖 Integrate AI-powered features:
  - AI chatbot to answer users’ art-related questions
  - AI-generated exhibition summaries or explanations
  - Personalized art recommendations based on user interests

---


## 🔒 Not Open Source (yet)

This is a **showcase-only repository**. The actual codebase contains sensitive configuration and private data and is not currently open-sourced.  
Please reach out if you're curious about the technical implementation or would like a live demo.

---

## 🙋‍♀️ Author

Diliao Xu  

[GitHub](https://github.com/DiliaoM) ・ [LinkedIn](https://www.linkedin.com/in/diliao-xu-8b162b192/)

